...
pub fn getTransactionByHash(cli: &Cli) -> Result<(), KissError> {
    ...
    let res = display_transaction(&v, &bcossdk.config,
                                  bcossdk.get_full_name().as_str(),get_opt_str(&cli.contractname).as_str());
    ...
    let contractABI = find_contract(bcossdk.get_full_name().as_str(),
...
pub fn getTransactionReceipt(cli: &Cli) -> Result<(), KissError> {
    ...
    let contractres = find_contract(bcossdk.get_full_name().as_str(),get_opt_str(&cli.contractname).as_str(), to, &bcossdk.config);
