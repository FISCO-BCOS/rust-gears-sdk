...
pub fn deploy(cli:&Cli) ->Result<(),KissError>{
    ...
    let binfile = format!("{}/{}.bin", bcos3client.config.common.contractpath, contractname.to_string());
    ...
    let res = ContractHistory::save_to_file(chf.as_str(),bcos3client.get_full_name().as_str(),contractname,address,blocknum as u64);
...
pub fn sendtx(cli:&Cli) ->Result<(),KissError>
{
    ...
    let address = ContractHistory::check_address_from_file(chfile.as_str(),bcos3client.get_full_name().as_str(),
                                                           opt.contract_name.as_str(),opt.address.as_str())?;

    println!("contract address is {}",&address.as_str());
    println!("method is {}",opt.method);
    let response = bcos3client.sendTransaction(address.as_str(),opt.method.as_str(),&opt.params,&contract)?;
    ...
    let receiptresult  = bcos3client.getTransactionReceipt(
        txhash,
        0,
        );
    ...
    match receiptresult {
        Ok(receipt) => {
            crate::console::console_utils::display_transaction_receipt(&receipt,&Option::from(&contract),&bcos3client.config);
        },
        Err(e) => {
...
pub fn call(cli:&Cli)->Result<(),KissError> {
    ...
    println!("contract address is {}", &address.as_str());
    let res = bcos3client.call(address.as_str(),opt.method.as_str(), &opt.params,&contract  )?;
