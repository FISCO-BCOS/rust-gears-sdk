...
pub fn sol_compile(contract_name: &str, configfile: &str) -> Result<Output, KissError> {
    ...
    println!("compile sol  {} ,use solc {},outputdir:{} options: {:?} ",
                 solfullpath.to_str().unwrap(), solc_path, config.common.contractpath.as_str(), options);
    let outputres = Command::new(solc_path).
        args(&options)
        .arg("-o").arg(config.common.contractpath.as_str())
    ...
    match outputres {
        Ok(out) => {
            if !out.status.success() {
                println!("compile command is NOT success! stderr:\n{}",String::from_utf8(out.stderr).unwrap());
                return kisserr!(KissErrKind::Error,"compile [{}] error status :{}",contract_name,out.status.to_string());
            }
            return Ok(out);
        },
