...
use fisco_bcos_rust_gears_sdk::bcos3sdk::bcos3sdkresponse::{Bcos3SDKResponse, bcos_sdk_c_struct_response};
use fisco_bcos_rust_gears_sdk::bcos3sdk::bcos3sdkwrapper::{BCOS3SDK_CALLBACK_FUNC, bcos_event_sub_subscribe_event,  bcos_sdk_get_last_error, bcos_sdk_get_last_error_msg, EventSubParam};
...
pub extern "C" fn demo_bcos3_event_callback(resp: *const bcos_sdk_c_struct_response) {
    ...
    unsafe {
        ...
        let contractabi = ContractABI::new_by_name(contractname.as_str(),
                                                   contextpath.as_str(),
                                                   &HashType::KECCAK).unwrap();
...
pub fn demo_event(cli: &Cli) -> Result<(), KissError> {
    unsafe {
        ...
        let cbfuture = Bcos3SDKFuture::create(Bcos3SDKFuture::next_seq(),
                                              "eventsub", format!("").as_str());
        let contractname = "HelloWorld";
        let methodname = "onset";
        let contractabi = ContractABI::new_by_name(contractname,
        ...
        let address = ContractHistory::get_last_from_file("./contracts/contracthistory.toml",
                                                          bcos3client.get_full_name().as_str(), contractname).unwrap();
        ...
        if mode.as_str() == "async"
        {
            ...
            bcos_event_sub_subscribe_event(bcos3client.sdk, str2p!(bcos3client.group.as_str()),
