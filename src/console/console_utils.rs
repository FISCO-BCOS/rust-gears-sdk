...
use fisco_bcos_rust_gears_sdk::bcossdk::bcosclientconfig::{BcosCryptoKind, ClientConfig};
use fisco_bcos_rust_gears_sdk::bcossdk::bcossdk::BcosSDK;
use fisco_bcos_rust_gears_sdk::bcossdk::commonhash::CommonHash;
...
use fisco_bcos_rust_gears_sdk::bcossdk::contracthistory::ContractRecord;
use crate::Cli;
...
pub fn cli_groupid(cli: &Cli) -> u32
{
    return match param_int(&cli.params, 0) {
        ...
        _ => {
            cli.default_config().unwrap().bcos2.groupid
        }
    };
}
...
///从历史里根据地址查找合约实例
pub fn find_contracthistory_byaddress(segment:&str,contractpath: &str,  address_input: &str) -> Result<ContractRecord, KissError>
{
...
pub fn find_contract(segment:&str,contractname:&str, address: &str, config: &ClientConfig) -> Result<ContractABI, KissError>
{
    if contractname.len() > 0
    {
            println!("load contract by name : {}", contractname);
            let contract = ContractABI::new_by_name(contractname,
            ...
            let chrecord =  find_contracthistory_byaddress(segment,config.common.contractpath.as_str(),
                                                  address)?;
            let splitres:Vec<&str> = chrecord.name.split("-chain").collect();
            let load_contract_name =splitres[0];
            let contract = ContractABI::new_by_name(load_contract_name,
                                            config.common.contractpath.as_str(),
                                            &CommonHash::crypto_to_hashtype(&config.common.crypto));
            println!("load contract {} by address : {}", chrecord.name,chrecord.address);
            return contract
    }
}
...
pub fn display_transaction_receipt(receipt_in: &JsonValue, contractopt: &std::option::Option<&ContractABI>, config: &ClientConfig)
{
...
pub fn display_transaction(v: &JsonValue, config: &ClientConfig, segment:&str,contractname:&str) -> Result<(), KissError>
{
