...
use crate::console::console_utils::{cli_groupid, display_transaction, display_transaction_receipt, find_contract, is_deploy_address, param_at, param_int};
use serde_json::{Value as JsonValue};
use fisco_bcos_rust_gears_sdk::bcossdk::kisserror::{KissErrKind, KissError};
use fisco_bcos_rust_gears_sdk::bcossdk::liteutils::get_opt_str;
use crate::cmdmap;
...
impl Bcos2Query{
    pub fn new()->Self{
        ...
        cmdmap!(query.cmdmap.cmd_func_map,   getTransactionByBlockNumberAndIndex);
...
pub fn getTransactionByHash(cli: &Cli) -> Result<(), KissError> {
    ...
    let res = display_transaction(&v["result"], &bcossdk.config, "bcos2",get_opt_str(&cli.contractname).as_str());
    ...
    let contractABI = find_contract("bcos2",get_opt_str(&cli.contractname).as_str(),
                                    to,
                                    &bcossdk.config
    )?;
...
pub fn getTransactionByBlockHashAndIndex(cli: &Cli) -> Result<(), KissError> {
    ...
    let receipt = bcossdk.getTransactionReceipt(hash)?;
    let to = v["result"]["to"].as_str().unwrap();
    let contract = find_contract("bcos2",get_opt_str(&cli.contractname).as_str(),
                                    to,
                                    &bcossdk.config
    )?;
...
pub fn getTransactionReceipt(cli: &Cli) -> Result<(), KissError> {
    ...
    let contractres = find_contract("bcos2",get_opt_str(&cli.contractname).as_str(), to, &bcossdk.config);
